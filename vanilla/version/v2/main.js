...
// ==========================================
const state = {
    user: null, // { username, score }
    score: 0,
    streak: 0,
    currentQuestion: null,
    view: 'quiz', // 'quiz' | 'leaderboard'
    leaderboard: [],
    lbStatus: 'idle', // 'idle' | 'loading' | 'success' | 'cooldown'
    lbCooldown: 0,
    quizUI: {
        selectedAnswer: null,
        isValidating: false,
        isTransitioning: false,
        correctAnswerText: null, // Dari socket
        animationFinished: false,
        pendingResult: null // Menyimpan hasil dari socket saat animasi belum selesai
    }
};
...
const logoutSuccessModal = document.getElementById('logout-success-modal'); // New
...
const logoutRefreshBtn = document.getElementById('logout-refresh-btn'); // New
const logoutTimer = document.getElementById('logout-timer'); // New
...
// NEW: Generic Fetch Function with Refresh Token Logic
async function fetchData(endpoint, method = 'GET', body = null, isRetry = false) {
    try {
        ...
        // Handle 401/403 - Refresh Token Logic
...
const socketService = {
    connect() {
        ...
        try {
            ...
            // Global listeners for robust handling
            socket.on("disconnect", (reason) => {
                ...
                if (reason === "io server disconnect") {
                    // Backend forced disconnect (Fatal)
                    ...
                    // If user is waiting for result (validating), reset UI immediately so they are not stuck
                    if (state.quizUI.isValidating) {
                        ...
                        renderQuiz(); // Redraw quiz to remove orange state
                    }
                }
            });
            ...
            socket.on("connect_error", async (err) => {
                ...
                // Attempt one auth refresh if connection fails (likely auth middleware)
                const res = await fetch(`${API_BASE_URL}/auth/refresh`, { method: 'POST' });
                if (res.ok) {
                    socket.connect(); // Retry manually after refresh
                } else {
                    // Fatal auth error on socket
                    state.user = null;
                    renderHeader();
                    openAuthModal();
    ...
    submitAnswer(answer) { if (socket && socket.connected) socket.emit("submit_answer", answer); }, // Simplified
...
async function handleAuthSubmit(e) {
    ...
    if (res.success) {
        if (isLoginMode) {
            ...
            if(userData.score) state.score = userData.score;
...
function openAuthModal() {
    ...
    // FIX: Always show close button now to allow placeholder mode
    authCloseBtn.classList.remove('hidden');
}

// Placeholder State Logic (Updated to 5 choices and "Jawaban")
...
function handleAnswer(choice) {
    ...
    if (state.user) {
        ...
        // Placeholder Logic: Just animate reset
            setTimeout(() => {
            state.quizUI.selectedAnswer = null;
            state.quizUI.isValidating = false;
            renderQuiz();
...
function renderHeader() {
    ...
    if (state.user) {
        ...
        `;
        // FIX: Listener now opens modal instead of logging out directly
...
function renderQuiz() {
    ...
    card.className = "w-full max-w-lg bg-dark-800/95 backdrop-blur-sm rounded-3xl shadow-xl shadow-black/30 flex flex-col overflow-hidden max-h-[85vh] border-2 transition-all duration-500 relative"; // Reset
...
function renderLeaderboard() {
    ...
    if (state.streak >= 10) {
        lbCard.className = "bg-dark-800 rounded-3xl shadow-xl shadow-red-900/30 overflow-hidden border border-red-900/50 flex flex-col h-[75vh] relative mb-4";
        ...
        lbCard.className = "bg-dark-800 rounded-3xl shadow-xl shadow-black/20 overflow-hidden border border-dark-700 flex flex-col h-[75vh] relative mb-4 transition-all duration-1000";
    ...
    if (state.lbStatus === 'loading') {
        btn.className = "w-full py-4 rounded-2xl transition-all flex items-center justify-center gap-2 font-bold shadow-md text-sm md:text-base bg-dark-800 text-slate-400 cursor-wait border border-dark-700";
        btn.textContent = "Updating...";
        btn.disabled = true;
    } else if (state.lbStatus === 'success') {
        btn.className = "w-full py-4 rounded-2xl transition-all flex items-center justify-center gap-2 font-bold shadow-md text-sm md:text-base bg-emerald-900/30 text-emerald-400 border border-emerald-800/50 shadow-emerald-900/20";
        btn.textContent = "Updated!";
        btn.disabled = false;
    } else if (state.lbStatus === 'cooldown') {
        btn.className = "w-full py-4 rounded-2xl transition-all flex items-center justify-center gap-2 font-bold shadow-md text-sm md:text-base bg-dark-900 text-slate-500 cursor-not-allowed border border-dark-800";
        btn.textContent = `Wait ${state.lbCooldown}s`;
        btn.disabled = true;
    } else {
        btn.className = "w-full py-4 rounded-2xl transition-all flex items-center justify-center gap-2 font-bold shadow-md text-sm md:text-base bg-dark-800 border border-dark-700 text-indigo-400 hover:bg-dark-700 hover:border-indigo-500/50 hover:shadow-lg active:scale-[0.98]";
    ...
    if (myRankData !== -1) {
        ...
        myRankRow.className = `border-t shrink-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.2)] px-2 transition-colors duration-1000 ${state.streak >= 10 ? 'bg-red-900/20 border-red-900/30' : 'bg-amber-900/20 border-amber-900/30'}`;
        
        myRankRow.innerHTML = `
            <table class="w-full table-fixed">
                <tbody class="text-sm md:text-base">
                    <tr class="${state.streak >= 10 ? 'text-red-400' : 'text-amber-400'}">
                        <td class="px-4 py-3 font-bold w-[15%] text-center">#${myRankData + 1}</td>
                        <td class="px-4 py-3 font-bold w-[60%] truncate">You</td>
                        <td class="px-4 py-3 text-right font-mono font-bold w-[25%]">${formatScore(displayList[myRankData].score)}</td>
                    </tr>
                </tbody>
            </table>
        `;
...
usernameInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
        e.preventDefault();
        if (isLoginMode) passwordInput.focus();
